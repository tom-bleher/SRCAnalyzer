...
class MainWindow(QMainWindow):
    ...
    def createCheckNNRegion(self):
        ...
        step_nn = 0.1
        ratios = []
        for np_val in np.arange(0.5, 2 + step, step):
            for pp_val in np.arange(0.5, 1.5 + step, step):
                print("np,pp = ", np_val, pp_val)
                found = False
                for nn_val in np.arange(1, 3 + step_nn, step_nn):
                    ...
                    r = get_ratios_from_pair_nums(pair_nums)
                    ratios.append(r)
                    # r[0] -> R(e,e'p), r[1] -> R(e,e'n), r[2] -> R(e,e'), r[3] -> R(e,e'np), r[4] -> R(e,e'pp)
                    if (abs(r[0] - 1.02) < prec and
                        ...
                        r[4] > 0):
                        found = True
                        found_ratios.append(r)
                        print("nn", nn_val)
                        break
                if found:
                    valid_x.append(np_val)
                    valid_y.append(pp_val)
                    print("found!")
        # instead of a scatter plot, display using plt.imshow()
        plt.figure()
