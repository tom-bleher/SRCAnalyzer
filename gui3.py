...
class MainWindow(QMainWindow):
    ...
    def check_nn_worker(self, np_range, pp_step, nn_step, prec):
        """Worker function to process a subset of np values"""
        local_valid_x = []
        local_valid_y = []
        local_found_ratios = []
        
        for np_val in np_range:
            for pp_val in np.arange(0.5, 1.5 + pp_step, pp_step):
                print(f"Processing np,pp = {np_val}, {pp_val}")
                found = False
                for nn_val in np.arange(1, 3 + nn_step, nn_step):
                    ...
                    if (abs(r[0] - 1.02) < prec and
                        ...
                        r[4] > 0):
                        found = True
                        local_found_ratios.append(r)
                        print("nn", nn_val)
                        break
                if found:
                    local_valid_x.append(np_val)
                    local_valid_y.append(pp_val)
                    print("found!")
        
        return local_valid_x, local_valid_y, local_found_ratios
    ...
    def createCheckNNRegion(self):
        ...
        # Create np value ranges for each process
        np_start, np_end = 0.5, 2.0
        num_cores = mp.cpu_count()
        np_step = 0.02
        np_values = np.arange(np_start, np_end + np_step, np_step)
        chunks = np.array_split(np_values, num_cores)
        
        # Create pool and run parallel processes
        with mp.Pool(num_cores) as pool:
            results = pool.starmap(self.check_nn_worker, 
                                 [(chunk, pp_step, nn_step, prec) for chunk in chunks])
        ...
        found_ratios = []
        for res in results:
            valid_x.extend(res[0])
            valid_y.extend(res[1])
            found_ratios.extend(res[2])

        # Plot results
        plt.figure()
        plt.imshow([valid_x], [valid_y])
        plt.xlim(0, 3)
        plt.ylim(0.5, 2)
        plt.xlabel("np48/np40")
        ...
        with open("points.csv", "w") as f:
            ...
            for (x_val, y_val, rat) in zip(valid_x, valid_y, found_ratios):
                f.write(f"{x_val},{y_val},{rat[0]},{rat[1]},{rat[2]},{rat[3]},{rat[4]}\n")
